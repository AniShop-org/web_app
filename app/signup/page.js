...
export default function Signup() {
  ...
  const handleSighnupSuccess = async (token) => {
    ...
    // Perform cart sync and address check
    try {
      // First sync the cart
      const syncSuccess = await syncLocalCartWithServer();
      console.log("Cart sync result:", syncSuccess);

      // Then check if user has an address
      ...
      const hasItemsInCart = localCartItems.length > 0;

      // Determine where to redirect
      if (syncSuccess && !hasAddress && hasItemsInCart) {
        router.push("/new-address?redirect=cart");
      } else if (hasItemsInCart && syncSuccess) {
        router.push("/cart");
      } else {
        router.push("/");
      }
      ...
      // If there was an error, still allow user to continue
      router.push("/");
    }
  };
    ...
    onSuccess: async (tokenResponse) => {
      try {
        ...
        try {
          ...
          const data = await response.json();
          if (response.ok) {
            handleSighnupSuccess(data.token);
          } else {
            setError(data.message || "Signup failed");
          }
        } catch (err) {
    ...
    <div className="flex flex-col md:flex-row w-full min-h-screen bg-[#191919] relative">
      ...
      <div className="w-full md:w-1/2 flex items-center justify-center relative z-10 pt-20 sm:pt-0 overflow-hidden">
        <div className="w-full max-w-xl p-8 flex flex-col min-h-screen md:min-h-0">
          ...
          <p className="text-[#808080] mb-6 text-sm sm:text-base">
          ...
          <div className="flex justify-center">
            <button
              className="bg-white text-black rounded-lg p-2.5 px-20 md:px-28 flex"
          ...
          <div className="flex items-center justify-center mb-6 mt-1 sm:gap-1.5 gap-1">
            <div className=" h-0.5 w-full bg-[#999999]" /> or{" "}
            <div className=" h-0.5 w-full bg-[#999999]" />
          </div>
          ...
          <p className="mt-6 text-[#808080] text-sm">
            ...
            <Link
              href="/login"
              className="text-[#FF3333] hover:text-[#E62E2E] underline"
            >
              Log in
            </Link>
          </p>
