...
export default function OTPVerification() {
  ...
  const handleSubmit = async (enteredOTP) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_SERVER_URL}/api/v1/user/auth/login-verify-otp`,
        {
          method: "POST",
  ...
  const handleResendOtp = async () => {
    ...
    try {
      setLoading(true);
      await fetch(
        `${process.env.NEXT_PUBLIC_SERVER_URL}/api/v1/user/auth/resend-otp`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
